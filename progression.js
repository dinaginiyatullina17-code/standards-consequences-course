...
function renderChoice(id) {
  ...
  fb.textContent = state.correct ? 'Верно. ' + cfg.explanation
    : state.done ? 'Неверно. ' + WRONG_REASONS[id][state.choice] + ' Верный вариант — «' + options[cfg.right].textContent.trim().replace(/[.!?]+$/, '') + '», потому что ' + CORRECT_REASONS[id] + '.'
...
function restoreSystem() {
  ...
  fb.className = 'feedback-box show ' + (state.done ? 'correct' : 'incorrect');
  fb.textContent = state.correct ? 'Верно. Наблюдение, реакция и проверка работают как единый цикл.' : state.done ? 'Неверно. ' + reasons + ' Верные действия — регулярно проверять соблюдение стандартов, сразу давать ОС по ФПР и возвращаться через несколько дней для проверки, потому что контроль должен включать обнаружение нарушения, своевременную реакцию и проверку усвоенного навыка.' : 'Подсказка: наблюдай в течение смены, реагируй в момент нарушения, а усвоение процедуры проверяй в работе через несколько дней.' + attemptsHint(state);
}
